...
const createProjectAction = async (project, other) => {
  ...
  if (!frame) {
    console.log('请写上您要用哪个框架开发~ 例如: dwj create project -f react(or vue2 3)');
    return
  }
  ...
  switch (frame) {
    case 'react':
      ...
      cloneFrameTemplate(reactAddress, project, frame)
      break
    case 'vue2':
      console.log('正在为您创建项目中~')
      cloneFrameTemplate(vue2Address, project, frame)
      break
    case 'vue3':
      console.log('正在为您创建项目中~')
      cloneFrameTemplate(vue3Address, project, frame)
    default:
