...
const { reactAddress, vueAddress } = require('../config/address')
...
const createProjectAction = async (project, other) => {
  ...
  if (!frame) {
    console.log('请写上您要用哪个框架开发~ 例如: dwj create project -f react(vue)');
    return
  }
  ...
  switch (frame) {
    ...
    case 'vue':
      console.log('正在为您创建项目中~')
      cloneFrameTemplate(vueAddress, project)
      break
    default:
...
const addComponentAction = async (name, dest) => {
  ...
  const componentPath = path.resolve(targetDest, `${name}.jsx`)
